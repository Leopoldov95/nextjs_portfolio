...
const Main = ({ active, idx }) => {
  const { cv_link } = jsonData;
  return (
    <header className={`container ${active === idx ? "active" : "hidden"}`}>
      {/* Header Content */}
      <div className=" grid grid-cols-2 min-h-screen h-full xs:grid-cols-1 xs:pb-24 md_s:grid-cols-1 md_s:pb-24">
        ...
        <div className="flex flex-col justify-center pr-72 lg:pr-36 xs:grid-rows-1 xs:pr-0 xs:w-[90%] xs:m-[0_auto] md_s:row-span-full md_s:pr-0 md_s:w-[90%] md_s:mx-auto md_s:my-0">
          {/* <!-- Name --> */}
          ...
          <p className="m-6 leading-8">
            ...
            specialize in HTML, CSS, and JavScript and use React as my preffered
            frontend framework. I also have a bit of knowledge in the backend
